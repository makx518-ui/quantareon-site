...
(function() {
    ...
    const CONFIG = {
        ...
        // 🕰️ СКОЛЬКО ТИШИНЫ ЖДЁМ, ПРЕЖДЕ ЧЕМ РЕШИТЬ «ЧЕЛОВЕК ДОГОВОРИЛ».
        // Кадр ≈ 32 мс. 15 ≈ 0.5 сек — ЕГО ЧИСЛО ОТ 01.08, при котором и был
        // тот самый реалтайм на Nova-3. Позже его подняли до 34 (вместе с
        // серверными endpointing 300→1000 и utterance_end 1000→2400) ради
        // склейки фразы с паузой — и скоростью заплатили. Он выбирает
        // скорость. ЗДЕСЬ НИЧЕГО НЕ ПОДНИМАТЬ.
    ...
    const TXT = {
        ru: {
            ...
            greetText:  'Приветствую тебя путник! Я Квантареон, голосовой помощник этого сайта. Спрашивай, что тебя интересует.'
        },
        en: {
        ...
        try {
            ...
            switch (d.type) {
                ...
                case 'audio_start':
                    ...
                    // ⛔ НЕ ЧИСТИМ здесь накопленный текст!
                    // Сервер шлёт текст ПЕРЕД озвучкой, поэтому к моменту
                    // прихода звука реплика уже частично собрана. Раньше ответ
                    // резался на много кусков и до окна долетали поздние, а
                    // когда фразы стали связными, ответ приходит одним куском —
                    // и очистка здесь стирала его целиком. Чистим только после
                    // вывода в окно, в audio_end.
                    break;
        ...
        try {
            ...
            scriptProc.onaudioprocess = (e) => {
                ...
                if (сейчас - микКогда > 4000) {
                    ...
                    // ⚠️ ПОКА ПОМОЩНИК ГОВОРИТ — ЧЕЛОВЕК МОЛЧИТ, И ЭТО НОРМА.
                    // Раньше счётчик тикал и во время ответа: послушал минуту
                    // про погоду — и получил «микрофон не слышит звука», хотя
                    // микрофон исправен. Ложная тревога, он на неё и поймал.
                    // Считаем тишину только когда очередь говорить за ним,
                    // и терпим дольше — 6 проверок это около полуминуты.
                    if (isPlaying) {
                        микТихо = 0;
                    } else if (микМакс < 0.001) {
                        микТихо++;
                        if (микТихо === 6 && typeof showToast === 'function') {
    ...
    function sendTyped(text) {
        ...
        // ⚠️ СВОЮ РЕПЛИКУ ЗДЕСЬ НЕ ПОКАЗЫВАЕМ. Сервер пришлёт её обратно
        // сообщением transcript_final, и окно нарисует её само — иначе
        // написанное появлялось ДВАЖДЫ. Он это поймал 08.08.
        // ⚠️ ВОСПРОИЗВЕДЕНИЕ ЗДЕСЬ НЕ ТРОГАЕМ ВООБЩЕ.
        // Пробовал глушить звук перед отправкой (stopPlayback + сброс
        // isBotSpeaking) — и голос пропал совсем: чистилась очередь, а
        // новый звук в неё уже не вставал. Он это поймал 08.08.
        // Просто отправляем текст: сервер сам оборвёт прежний ответ и
        // начнёт новый, как при обычной речи.
