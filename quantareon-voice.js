...
(function() {
    ...
    function sendTyped(text) {
        ...
        // ⚠️ СЕРВЕРУ «ПЕРЕБИВАЮ» НЕ ШЛЁМ. Этот сигнал может дойти ПОЗЖЕ
        // самого текста, и тогда сервер оборвёт уже начатый ответ — выйдет
        // тишина. Он это поймал 08.08. Достаточно заглушить звук у себя:
        // сервер, получив новую реплику, сам начнёт отвечать заново.
        if (isBotSpeaking || isPlaying) {
            stopPlayback();
            isBotSpeaking = false;
        }
